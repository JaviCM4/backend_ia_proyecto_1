...
const PESOS = {
  ...
  lab_periodos_correctos: 5,         // lab con exactamente noPeriodosLab periodos contiguos
};
/*
const PESOS = {
...
function calcularPremioCoherenciaSeccion(genes: Gen[], ctx: ContextoGA): number {
  ...
  for (const { teoria, lab } of porSeccion.values()) {
    ...
    // +5 lab tiene el número correcto de periodos contiguos
    if (lab) {
      const curso = ctx.indiceCursos.get(lab.cursoId);
      const expectedPeriods = curso?.noPeriodosLab ?? 2;
      const actualPeriods = 1 + (lab.franjasExtra?.length ?? 0);
