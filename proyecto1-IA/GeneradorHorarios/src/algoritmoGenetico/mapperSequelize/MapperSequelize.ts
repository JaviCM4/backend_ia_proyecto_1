...
async function cargarDocentes(): Promise<DocenteGA[]> {
  ...
  return rows.map((r) => ({
    ...
    // Si hora_salida es null, el docente no tiene restricción → puede dar clase todo el día
    horaEntrada: r.hora_entrada ? toHHMM(r.hora_entrada) : '07:00',
    horaSalida:  r.hora_salida  ? toHHMM(r.hora_salida)  : '21:10',
...
export async function cargarContexto(
  ...
  // Nota: usamos periodosRows (datos reales de BD) no periodos (siempre [] por diseño)
